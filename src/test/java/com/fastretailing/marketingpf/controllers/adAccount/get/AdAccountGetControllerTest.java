...
class AdAccountGetControllerTest extends BaseSpringBootTest {

    @MockBean
    private AdAccountGetController adAccountGetController;
    ...
    public void expectingFindSuccess() throws Exception {
        ...
        AdAccountGetResponse response = new AdAccountGetResponse(adAccount);
        Mockito.doReturn(new ResponseEntity<Object>(response, HttpStatus.OK)).when(adAccountGetController).get(Mockito.anyLong());

        mvc.perform(get("/api/marketingpf/v1/fr/jp/ad_account/1001").contentType(MediaType.APPLICATION_JSON))
